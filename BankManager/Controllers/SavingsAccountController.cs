            ...
            using (var connection = new SQLiteConnection(Startup.ApplicationConnectionString))
            {
                ...
                using (SQLiteTransaction transaction = connection.BeginTransaction())
                {
                    using (var command = connection.CreateCommand())
                    {
                        ...
                        command.Parameters.Add("@accountTypeId", DbType.Int32).Value = (int)AccountType.Savings;
...
where id = @id";
                        ...
                        if (account.Interest.HasValue)
                            command.Parameters.Add("@interest", DbType.Double).Value = account.Interest;
                        else
                            command.Parameters.Add("@interest", DbType.Double).Value = DBNull.Value;
